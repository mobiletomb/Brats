...
class BratsDatasetT2(Dataset):
    def __init__(self, df: pd.DataFrame, phase: str='test', is_resize: bool=False):
        self.df = df
        self.phase = phase
        self.augmentations = get_augmentations(phase)
        self.data_types = ['_flair.nii.gz', '_t2.nii.gz']
        self.is_resize = is_resize
...
def get_paired_dataloader(
        dataset_t1: torch.utils.data.Dataset,
        dataset_t2: torch.utils.data.Dataset,
        path_to_csv: str,
        phase: str,
        fold: int = 0,
        batch_size: int = 1,
    ...
    df = train_df if phase == 'train' else val_df
    dataset_t1 = BratsDatasetT1(df, phase)
    dataset_t2 = BratsDatasetT2(df, phase)
    dataloader_t1_pair = DataLoader(
        dataset_t1,
        batch_size=batch_size,
        num_workers=num_workers,
        pin_memory=True,
    ...
    return zip(dataloader_t1_pair, dataloader_t2_pair)
