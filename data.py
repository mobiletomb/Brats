...
if __name__ == '__main__':
    ...
    trans_crop = []
    for i in range(4):
        hori = HorizontalFlip(p=1)(image=img[i])['image']
        trans_hori.append(hori)
        cro = CenterCrop(p=1, width=100, height=100)(image=img[i])['image']
        trans_crop.append(cro)

    for i in range(4):
        plt.subplot(1, 4, i+1)
        plt.axis('off')
        plt.imshow(trans_hori[i], 'gray')
        # plt.imshow(trans_crop[i], 'gray')
    plt.show()
    # print((trans == img).all())

    """
    img_patch = rearrange(img, '(h p_h) (w p_w) -> (h w) p_h p_w', p_h=60, p_w=60)
    print(img_patch.shape)
    img_patch = montage(img_patch, padding_width=10)
    print(img_patch.shape)
    """
